...
function UpdateSteamProfiles($database, $apikey, $steamid, $uid)
{
    ...
    $url = "http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?key=$apikey&steamids=$steamid";
    ...
    $url = "http://api.steampowered.com/IPlayerService/GetBadges/v1/?key=$apikey&steamid=$steamid";
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, $url);
    curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
    curl_setopt($curl, CURLOPT_HEADER, 0);
